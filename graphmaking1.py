...
def graph1():
    ...
    if c=="month":
        ...
        l = input(" "*50+" Enter what you want to see if you want to see as no.of sell of book i or total amount that you get in that month ")
        if l=="sell":
            jkl = ["January","february","march","april","june","july","august","september","october","november","december"]
            plt.plot()
            plt.bar(monk,monthly, tick_label=monthly,width=0.8, color=['red', 'green'])
            plt.xlabel("no. of days")
            plt.ylabel("sells on that day")
            plt.title("This is the data of " +str(jkl[month]))
            plt.show()
        if l=="totalamount":
            jkl = ["January", "february", "march", "april", "june", "july", "august", "september", "october","november", "december"]
            plt.bar(monk, totalrupees1, tick_label=monthly, width=0.8, color=['red', 'green'])
            plt.xlabel("no. of days")
            plt.ylabel("sells on that day")
            plt.title("This is the data of month  " + str(jkl[month]))
            plt.show()
    if c=="year":
        ...
        print(" "*50+"[sell] [totalamount] ")
        l = input(" Enter what you want to see in graph [no.of book sell vs month ] than write sell else write totalamount to find total income that youb had earned in this year ")
        if l == "sell":
            plt.bar(yearly, pricecount, tick_label=yearly, width=0.8, color=['red', 'green'])
            plt.xlabel("no. of month")
            plt.ylabel("sells on that day")
            plt.title("This is the data of " + str(int(d)))
            plt.show()
        if l == "totalamount":
            plt.bar(yearly ,yearcount , tick_label=yearly, width=0.8, color=['red', 'green'])
            plt.xlabel("no. of month")
            plt.ylabel("sells on that day")
            plt.title("This is the data of   " + str(int(d)))
            plt.show()
